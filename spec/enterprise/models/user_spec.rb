  ...
  describe 'before validation for pricing plans' do
    ...
    context 'when pricing plan is not premium' do
      before do
        ...
        allow(WaselchatHub).to receive(:pricing_plan_quantity).and_return(0)
      end
    ...
    context 'when pricing plan is premium' do
      before do
        allow(WaselchatHub).to receive(:pricing_plan).and_return('premium')
      end

      context 'when the user limit is reached' do
        it 'adds an error when trying to create a user' do
          allow(WaselchatHub).to receive(:pricing_plan_quantity).and_return(1)
          new_user.valid?
      ...
      context 'when the user limit is not reached' do
        it 'does not add an error to the user' do
          allow(WaselchatHub).to receive(:pricing_plan_quantity).and_return(3)
          new_user.valid?

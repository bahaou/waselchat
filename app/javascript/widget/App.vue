...
export default {
  ...
  mounted() {
    const { websiteToken, locale, widgetColor } = window.waselchatWebChannel;
  ...
  methods: {
    ...
    setLocale(localeWithVariation) {
      ...
      const { enabledLanguages } = window.waselchatWebChannel;
    ...
    registerCampaignEvents() {
      ...
      emitter.on('execute-campaign', campaignDetails => {
        ...
        const { websiteToken } = window.waselchatWebChannel;

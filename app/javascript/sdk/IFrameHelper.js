...
import { WASELCHAT_ERROR, WASELCHAT_READY } from '../widget/constants/sdkEvents';
...
export const IFrameHelper = {
  ...
  createFrame: ({ baseUrl, websiteToken }) => {
    ...
    iframe.id = 'waselchat_live_chat_widget';
    iframe.style.visibility = 'hidden';

    let holderClassName = `woot-widget-holder woot--hide woot-elements--${window.$waselchat.position}`;
    if (window.$waselchat.hideMessageBubble) {
      holderClassName += ` woot-widget--without-bubble`;
    }
    if (isFlatWidgetStyle(window.$waselchat.widgetStyle)) {
  ...
  getAppFrame: () => document.getElementById('waselchat_live_chat_widget'),
  getBubbleHolder: () => document.getElementsByClassName('woot--bubble-holder'),
  sendMessage: (key, value) => {
    const element = IFrameHelper.getAppFrame();
    element.contentWindow.postMessage(
      `waselchat-widget:${JSON.stringify({ event: key, ...value })}`,
      '*'
    );
  },
  initPostMessageCommunication: () => {
    window.onmessage = e => {
      if (
        ...
        e.data.indexOf('waselchat-widget:') !== 0
      ) {
        return;
      }
      const message = JSON.parse(e.data.replace('waselchat-widget:', ''));
  ...
  setupAudioListeners: () => {
    const { baseUrl = '' } = window.$waselchat;
  ...
  events: {
    loaded: message => {
      updateAuthCookie(message.config.authToken, window.$waselchat.baseDomain);
      window.$waselchat.hasLoaded = true;
      const campaignsSnoozedTill = Cookies.get('cw_snooze_campaigns_till');
      IFrameHelper.sendMessage('config-set', {
        locale: window.$waselchat.locale,
        position: window.$waselchat.position,
        hideMessageBubble: window.$waselchat.hideMessageBubble,
        showPopoutButton: window.$waselchat.showPopoutButton,
        widgetStyle: window.$waselchat.widgetStyle,
        darkMode: window.$waselchat.darkMode,
        showUnreadMessagesDialog: window.$waselchat.showUnreadMessagesDialog,
      ...
      if (window.$waselchat.user) {
        IFrameHelper.sendMessage('set-user', window.$waselchat.user);
      }
      ...
      if (!window.$waselchat.resetTriggered) {
        dispatchWindowEvent({ eventName: WASELCHAT_READY });
      }
    },
    error: ({ errorType, data }) => {
      dispatchWindowEvent({ eventName: WASELCHAT_ERROR, data: data });
    ...
    setBubbleLabel(message) {
      setBubbleText(window.$waselchat.launcherTitle || message.label);
    },

    setAuthCookie({ data: { widgetAuthToken } }) {
      updateAuthCookie(widgetAuthToken, window.$waselchat.baseDomain);
    },

    setCampaignReadOn() {
      updateCampaignReadStatus(window.$waselchat.baseDomain);
    },
  ...
  onLoad: ({ widgetColor }) => {
    ...
    iframe.setAttribute('id', `waselchat_live_chat_widget`);

    if (IFrameHelper.getBubbleHolder().length) {
      return;
    }
    createBubbleHolder(window.$waselchat.hideMessageBubble);
    onLocationChangeListener();

    let className = 'woot-widget-bubble';
    let closeBtnClassName = `woot-elements--${window.$waselchat.position} woot-widget-bubble woot--close woot--hide`;

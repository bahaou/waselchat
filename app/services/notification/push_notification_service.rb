class Notification::PushNotificationService
  ...
  def perform
    ...
    notification_subscriptions.each do |subscription|
      ...
      send_push_via_waselchat_hub(subscription)
    end
  end
  ...
  def send_browser_push(subscription)
    ...
    WaselchatExceptionTracker.new(e, account: notification.account).capture_exception
    true
  end
  ...
  def send_push_via_waselchat_hub(subscription)
    return if firebase_credentials_present?
    return unless waselchat_hub_enabled?
    return unless subscription.fcm?

    WaselchatHub.send_push(fcm_options(subscription))
  end

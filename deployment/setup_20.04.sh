...
# Description: Install and manage a Waselchat installation.
...
WASELCHAT_HUB_URL="https://hub.2.waselchat.com/events"
...
function exit_handler() {
  if [ "$?" -ne 0 ] && [ "$u" == "n" ]; then
   echo -en "\nSome error has occured. Check '/var/log/waselchat-setup.log' for details.\n"
   exit 1
  fi
}
...
function get_domain_info() {
  read -rp 'Enter the domain/subdomain for Waselchat (e.g., waselchat.domain.com): ' domain_name
...
serve Waselchat at https://$domain_name.
...
function create_cw_user() {
  if ! id -u "waselchat"; then
    adduser --disabled-password --gecos "" waselchat
  fi
}
...
function save_pgpass() {
  mkdir -p /opt/waselchat/config
  file="/opt/waselchat/config/.pg_pass"
  if ! test -f "$file"; then
    echo $pg_pass > /opt/waselchat/config/.pg_pass
  fi
}
...
##############################################################################
# Configure postgres to create waselchat db user.
...
function run_db_migrations(){
  sudo -i -u waselchat << EOF
  cd waselchat
  RAILS_ENV=production POSTGRES_STATEMENT_TIMEOUT=600s bundle exec rails db:waselchat_prepare
EOF
}

##############################################################################
# Setup Waselchat systemd services and cwctl CLI
# Globals:
#   None
# Arguments:
#   None
...
function configure_systemd_services() {
  ...
  cp /home/waselchat/waselchat/deployment/waselchat /etc/sudoers.d/waselchat
  cp /home/waselchat/waselchat/deployment/setup_20.04.sh /usr/local/bin/cwctl
...
function setup_ssl() {
  ...
  wget https://raw.githubusercontent.com/waselchat/waselchat/develop/deployment/nginx_waselchat.conf
  cp nginx_waselchat.conf /etc/nginx/sites-available/nginx_waselchat.conf
  certbot certonly --non-interactive --agree-tos --nginx -m "$le_email" -d "$domain_name"
  sed -i "s/waselchat.domain.com/$domain_name/g" /etc/nginx/sites-available/nginx_waselchat.conf
  ln -s /etc/nginx/sites-available/nginx_waselchat.conf /etc/nginx/sites-enabled/nginx_waselchat.conf
  systemctl restart nginx
  sudo -i -u waselchat << EOF
  cd waselchat
  sed -i "s/http:\/\/0.0.0.0:3000/https:\/\/$domain_name/g" .env
EOF
  systemctl restart waselchat.target
}
...
function setup_logging() {
  touch /var/log/waselchat-setup.log
  LOG_FILE="/var/log/waselchat-setup.log"
}
...
***************************************************************************
Woot! Woot!! Waselchat server installation is complete.
The server will be accessible at https://$domain_name

Join the community at https://waselchat.com/community?utm_source=cwctl
***************************************************************************
...
function cwctl_message() {
  echo $'\U0001F680 Try out the all new Waselchat CLI tool to manage your installation.'
...
function get_cw_version() {
  CW_VERSION=$(curl -s https://app.waselchat.com/api | python3 -c 'import sys,json;data=json.loads(sys.stdin.read()); print(data["version"])')
}
...
***************************************************************************
              Waselchat Installation (v$CW_VERSION)
***************************************************************************

For more verbose logs, open up a second terminal and follow along using,
'tail -f /var/log/waselchat-setup.log'.

EOF

  sleep 3
  read -rp 'Would you like to configure a domain and SSL for Waselchat?(yes or no): ' configure_webserver
  ...
  echo "➥ 6/9 Installing Waselchat. This takes a long while."
  setup_waselchat &>> "${LOG_FILE}"
...
https://www.waselchat.com/docs/deployment/deploy-waselchat-in-linux-vm?utm_source=cwctl
...
function get_console() {
  sudo -i -u waselchat bash -c " cd waselchat && RAILS_ENV=production bundle exec rails c"
}
...
function help() {
  ...
  cat <<EOF
Usage: cwctl [OPTION]...
Install and manage your Waselchat installation.
...
Installation/Upgrade:
  -i, --install             Install the latest stable version of Waselchat
  -I                        Install Waselchat from a git branch
  -u, --upgrade             Upgrade Waselchat to the latest stable version
...
Management:
  ...
  -l, --logs                View logs from Waselchat. Supported values include web/worker.
  -r, --restart             Restart Waselchat server
...
Report bugs at https://github.com/waselchat/waselchat/issues
Get help, https://waselchat.com/community?utm_source=cwctl

EOF
}

##############################################################################
# Get Waselchat web/worker logs (-l/--logs)
# Globals:
#   None
# Arguments:
#   None
...
function get_logs() {
  if [ "$SERVICE" == "worker" ]; then
    journalctl -u waselchat-worker.1.service -f
  fi
  if [ "$SERVICE" == "web" ]; then
    journalctl -u waselchat-web.1.service -f
  fi
}
...
function upgrade_redis() {
  ...
  echo "Upgrading Redis to v7+ for Rails 7 support(Waselchat v2.17+)"
...
function upgrade() {
  cwctl_upgrade_check
  get_cw_version
  echo "Upgrading Waselchat to v$CW_VERSION"
  sleep 3
  upgrade_prereq
  upgrade_redis
  upgrade_node
  ...
  # Navigate to the Waselchat directory
  cd waselchat
...
EOF
  ...
  # Restart the waselchat server
  systemctl restart waselchat.target

}

##############################################################################
# Restart Waselchat server (-r/--restart)
# Globals:
#   None
# Arguments:
#   None
...
function restart() {
  systemctl restart waselchat.target
  systemctl status waselchat.target
}
...
function report_event() {
  ...
  curl -X POST -H "Content-Type: application/json" -d "$data" "$WASELCHAT_HUB_URL" -s -o /dev/null
}
...
function get_installation_identifier() {
  ...
  installation_identifier=$(sudo -i -u waselchat << "EOF"
  cd waselchat
...
function cwctl_upgrade_check() {
    ...
    local remote_version_url="https://raw.githubusercontent.com/waselchat/waselchat/master/VERSION_CWCTL"
...
function upgrade_cwctl() {
    wget https://get.waselchat.app/linux/install.sh -O /usr/local/bin/cwctl > /dev/null 2>&1 && chmod +x /usr/local/bin/cwctl
}

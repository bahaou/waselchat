class WaselchatHub
  BASE_URL = ENV.fetch('WASELCHAT_HUB_URL', 'https://hub.2.waselchat.com')
  ...
  def self.support_config
    {
      support_website_token: InstallationConfig.find_by(name: 'WASELCHAT_SUPPORT_WEBSITE_TOKEN')&.value,
      support_script_url: InstallationConfig.find_by(name: 'WASELCHAT_SUPPORT_SCRIPT_URL')&.value,
      support_identifier_hash: InstallationConfig.find_by(name: 'WASELCHAT_SUPPORT_IDENTIFIER_HASH')&.value
    }
  end
  ...
  def self.instance_config
    {
      ...
      installation_version: Waselchat.config[:version],
  ...
  def self.sync_with_hub
    begin
      ...
      WaselchatExceptionTracker.new(e).capture_exception
    end
    parsed_response
  end
